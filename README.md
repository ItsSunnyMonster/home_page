<!--
SPDX-FileCopyrightText: 2025 SunnyMonster

SPDX-License-Identifier: CC-BY-SA-4.0
-->

# [Home Page](https://home.farkyouinc.fyi)

## How to display other timezones

Visit the URL with this format:

```
https://home.farkyouinc.fyi/?tz=[insert timezones here]
```

The TZ parameter should be formatted like this: Name:Timezone,Name2:Timezone2.

A list of all valid timezones can be found [here](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) (Look for the TZ identifier section).

**IMPORTANT**: Replace any forward slashes in the TZ identifier with backslashes (\\).

## License

This repository is [reuse compliant](https://reuse.software). Use the reuse tool or check file headers for per file licensing information.

To summarise, the code in this repository is licensed under the MIT license, configuration files are open to the public domain, and this README is licensed under the Creative Commons Attribution ShareAlike license.
